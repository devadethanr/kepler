...
from dashboard.pages.failed_events import failed_events


def index() -> rx.Component:
    return layout(command_center())


app = rx.App(style=global_styles)
app.add_page(
    index, route="/", title="Kepler Command Center", on_load=GlobalState.fetch_initial_data
)
app.add_page(
    portfolio,
    route="/portfolio",
    title="Portfolio | Kepler",
    on_load=GlobalState.fetch_initial_data,
)
app.add_page(
    research, route="/research", title="Research | Kepler", on_load=GlobalState.fetch_initial_data
)
app.add_page(
    approvals,
    route="/approvals",
    title="Approvals | Kepler",
    on_load=GlobalState.fetch_initial_data,
)
app.add_page(
    knowledge,
    route="/knowledge",
    title="Knowledge Graph | Kepler",
    on_load=GlobalState.fetch_initial_data,
)
app.add_page(
    activity,
    route="/activity",
    title="Agent Activity | Kepler",
    on_load=GlobalState.fetch_initial_data,
)
app.add_page(
    failed_events,
    route="/failed-events",
    title="Failed Events | Kepler",
    on_load=GlobalState.fetch_initial_data,
)
