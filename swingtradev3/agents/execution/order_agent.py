...
class OrderExecutionAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx) -> AsyncGenerator[Event, None]:
        ...
        if not state_payload:
            yield Event(
                author=self.name, 
                content=types.Content(role="assistant", parts=[types.Part(text="No active state found, skipping execution")])
            )
            return
        ...
        for approval in approvals:
            ...
            risk_decision = risk_tool.check_risk(state, score, entry_price, stop_price, target_price)
            ...
            if risk_decision["approved"]:
                ...
                await alerts_tool.send_alert(f"⚠️ Risk check failed for {ticker} post-approval: {risk_decision['reason']}")
        ...
        yield Event(
            ...
            content=types.Content(
                role="assistant",
                parts=[types.Part(text=f"Execution completed. Orders placed: {executed_count}")]
            ),
        )
