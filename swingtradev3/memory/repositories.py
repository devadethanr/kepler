...
class MemoryRepository:
    ...
    def _policy_overlay_payload(self, row: PolicyOverlayRow) -> dict[str, Any]:
        return {
    ...
    def get_policy_overlay(self, overlay_id: str) -> dict[str, Any] | None:
        row = self.session.get(PolicyOverlayRow, overlay_id)
        if row is None:
            return None
        return self._policy_overlay_payload(row)
    ...
    def list_policy_overlays(
        ...
        *,
        ...
        bounded_limit = max(1, min(limit, 500))
        query = select(PolicyOverlayRow).order_by(
            PolicyOverlayRow.updated_at.asc(),
            PolicyOverlayRow.created_at.asc(),
        )
        if status:
            query = query.where(PolicyOverlayRow.status == status)
        if key:
            query = query.where(PolicyOverlayRow.key == key)
        rows = self.session.scalars(query.limit(bounded_limit)).all()
        return [self._policy_overlay_payload(row) for row in rows]
    ...
    def upsert_policy_overlay(
        ...
        *,
        ...
        row = self.session.get(PolicyOverlayRow, overlay_id)
        if row is None:
            row = PolicyOverlayRow(overlay_id=overlay_id)
            self.session.add(row)
